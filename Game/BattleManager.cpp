...
Phase BattleManager::BeforeBattle(Character& player)
{
    ...
    ui.UpdateScene(false, "");
    Line();
    ui.PrintLog("\033[36m[탐험]\033[0m 미궁을 걷고 있습니다... 아무키나 입력하십시오.", 0);
...
{
    ui.ShowPlayerDead();
    SoundManager::GetInstance()->PlayBGM("Assets/Sound/8_bit_defeated.wav", false);
...
{
    SoundManager::GetInstance()->PlayBGM("Assets/Sound/8_bit_positive_long.wav", false);
...
void BattleManager::UseRandomItem(Character& player)
{
    ...
    if (items.empty())
    {
        ui.PrintLog("\033[32m[아이템 사용 시도]\033[0m 인벤토리가 텅텅 비었네요!!");
        return;
    }
    ...
    if (ResultOfUseItemPercent <= 30 && usableItems.size() > 0)
    {
        ...
        {
            SoundManager::GetInstance()->PlaySFX("Assets/Sound/08_human_charge_1.wav", "HealthPotion");
            ui.PrintLog("\033[32m[Health Potion]\033[0m Health: " + std::to_string(beforeHealth) + " -> " + std::to_string(player.Health), 500);
        }
